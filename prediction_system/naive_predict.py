...
def naive_predict_user(rating, users_count, products_count, distance_matrix, train_data_matrix):
...
def naive_predict_product(rating, users_count, products_count, distance_matrix, train_data_matrix):
    top_similar_ratings = np.zeros((products_count, rating, users_count))

    for i in range(products_count):
        top_sim_products = distance_matrix[i].argsort()[1:rating + 1]
